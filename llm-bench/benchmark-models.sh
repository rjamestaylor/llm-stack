...
# Get llm-bench directory and load configuration
...
REPORTS_DIR="$REPORTS_BASE_DIR/$SESSION_TIMESTAMP"
mkdir -p "$REPORTS_DIR"
...
# Get available models from Ollama using list-models script
get_available_models() {
    echo "Fetching available models from Ollama..."
    
    # Get the path to the list-models script using the helper function from config.sh
    LIST_MODELS_SCRIPT=$(get_script_path "list-models.sh")
    
    if [ -n "$LIST_MODELS_SCRIPT" ] && [ -f "$LIST_MODELS_SCRIPT" ]; then
        echo -e "${BLUE}Using list-models script: $LIST_MODELS_SCRIPT${NC}"
        ...
        done < <("$LIST_MODELS_SCRIPT" 2>/dev/null)
        ...
        if [ ${#models[@]} -gt 0 ]; then
            ...
            echo -e "${YELLOW}No models found from list-models script, trying API fallback...${NC}"
            models_from_api
        fi
    else
        echo -e "${YELLOW}list-models script not found, using API fallback...${NC}"
        models_from_api
    fi
...
benchmark_model_prompt() {
    ...
    local pid_file="$SCRIPT_DIR/tmp/cpu_samples_${safe_model_name}_${safe_prompt_name}.txt"
...
display_results() {
    ...
    echo "python $SCRIPT_DIR/visualize_benchmarks.py --summary-path '${reports_dir}/summary.csv' --output-dir '${reports_dir}'"
    echo ""
    ...
    echo ""
    echo "Example: python $SCRIPT_DIR/visualize_benchmarks.py --all"
    echo ""
    # NEW: Visualization script call for GPU metrics
    if [ "$enable_gpu_metrics" = true ]; then
        echo -e "\n\033[1;33mTo visualize GPU metrics:\033[0m"
        echo "Run: python $SCRIPT_DIR/visualize_benchmarks.py --summary-path '${REPORTS_DIR}/summary.csv' --output-dir '${REPORTS_DIR}' --include-gpu"
        echo ""
