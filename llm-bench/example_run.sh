...
if [ "$SESSION" = "sample" ]; then
    SUMMARY_PATH="$REPORTS_BASE_DIR/sample/sample_summary.csv"
    OUTPUT_DIR="$REPORTS_BASE_DIR/sample"
    echo -e "${YELLOW}Using sample data${NC}"
else
    # Use the provided session timestamp
    SUMMARY_PATH="$REPORTS_BASE_DIR/${SESSION}/summary.csv"
    OUTPUT_DIR="$REPORTS_BASE_DIR/${SESSION}"
...
python3 "$SCRIPT_DIR/visualize_benchmarks.py" \
